...
import { selectSomethingFromCookies } from '../../lib/selector'
...
function* setCookie(payload) {
  try {
    ...
    const isSetSuccess = yield select(selectSomethingFromCookies(Object.keys(payload)[0]))
    if(isSetSuccess) {
...
function* removeCookie() {
  try {
    ...
    const token = yield select(selectSomethingFromCookies('token'))
    if(token === "") {
